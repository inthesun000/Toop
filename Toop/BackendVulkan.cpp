...
void BackendVulkan::CreateGraphicsPipeline()
{
	...
	*/
	...
	auto bindingDescriptions = AWVertex::GetVertexBindingDescription();
	auto attributeDescriptions = AWVertex::GetVertexAttributeDescription();
...
void BackendVulkan::GenerateMipmaps(VkImage image, VkFormat imageFormat, int32_t width, int32_t height, uint32_t mipLevels)
{
...
VkSampleCountFlagBits BackendVulkan::GetMaxUsableSampleCount()
{
...
//	std::unordered_map<AWVertex, uint32_t> uniqueVerties = {};
	...
	for (auto& shape : shapes)
	{
		for (auto& index : shape.mesh.indices)
		{
			...
			/*
			* TODO: Check overlap vertex, and Remove overlap vertex
			* 1. Get AABB from shape
			* 2. Normailze all vertex use AABB (x/width, y/height, z/depth)
			* 3. 16bit convert integer
			* 4. Remove same integer
			* 5. convert floating point
			* 6. Scaling vertex use AABB
			*/
		}
	}
}
...
void BackendVulkan::CreateTextureImage()
{
	...
	CreateImage(texWidth, texHeight, textureMipLevels, VK_SAMPLE_COUNT_1_BIT, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_TILING_OPTIMAL,
...
void BackendVulkan::CreateImage(uint32_t width, uint32_t height, uint32_t mipLevels,
								VkSampleCountFlagBits count, VkFormat format,
...
void BackendVulkan::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels)
{
	VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
...
VkImageView BackendVulkan::CreateImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels)
{
...
void BackendVulkan::CreateColorResource()
{
